  ...
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-3 rounded-2xl bg-panel px-4 py-3">
      ...
      <button
        v-if="!readOnly"
        type="button"
        class="tap-target relative shrink-0 rounded-lg px-2 py-1 font-mono text-label tracking-label whitespace-nowrap text-clue-ink uppercase transition-opacity duration-100 ease-micro hover:opacity-80"
        @click="$emit('choose')"
      >
