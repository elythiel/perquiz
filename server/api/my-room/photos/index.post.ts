...
import { MAX_UPLOAD_BYTES, sniffImageType, storePhoto } from '../../../utils/photos'
...
export default defineEventHandler(async (event) => {
  ...
  const next = db.select({ count: sql<number>`count(*)` }).from(photos)
    .where(eq(photos.userId, userId)).get()?.count ?? 0

  db.insert(photos)
    .values({ userId, filename: stored.name, position: next })
    .run()

  setResponseStatus(event, 201)
  return { name: stored.name, position: next }
})
